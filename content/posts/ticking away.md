...
driver->velocity = math_hypot((int)driver->velocityX,(int)driver->velocityY) << 16;
```
This is the only place the game converts from `velocityX` and `velocityY` to `velocity`. Remember how I mentioned that only `velocityX` and `velocityY` were halved when we hit a wall? This is *supposed* to handle that. However, when we are airborne, the driverState isn't `NORMAL` and is instead `AIRBORNE`, so we never hit the DriverCollision function that is supposed to change `velocity`. This means that `velocityX` and `velocityY` get totally ignored, and eventually get set based on `velocity` instead of the other way around. In essence, throwing away the speed loss we should get for hitting a wall, and allowing us to carry all of our speed through the wall hit. This is why a mushroom boost isn't needed for ticking. We enter the ticking state as long as we have hopped into a wall, regardless of speed.
